...
- **Component media-prop export timing**: In `canvasExport.ts`, video frame capture for component `media` props is keyed per component clip instance and prop key (`${clipId}:${propKey}`), not just media path, so multiple components that reference the same source video stay frame-accurate.
- **Component clip media-time semantics**: Component clip `currentTime` (preview + export) uses clip media time (`trimStart + (globalTime - startTime)`), matching video clip trim behavior.
- **Render range (partial export)**: Store fields `renderRangeStart`/`renderRangeEnd` define in/out points. Export modal offers three modes: Full Timeline, Time Range (uses in/out markers), Selected Clips (uses `selectedClipIds` bounding range). `exportToVideo()` accepts `renderStart`/`renderEnd` params; audio mixdown clips are clamped and offset accordingly. Timeline markers (I/O keys, draggable, right-click context menu) visualise the range.
- **Decoded-frame capture in export**: After seek, `canvasExport.ts` waits for a decoded video frame (`requestVideoFrameCallback` with timeout fallback) before rasterization to avoid stale-frame jitter.
...
- During export, never de-duplicate component media-prop video captures by source path alone; de-duplicate by clip+prop binding so concurrent component instances using the same video do not share an incorrect timeline frame.
- Component clip `trimStart` must be applied consistently in preview, component media-prop frame capture, and component media-prop audio mixdown, or component-contained videos drift from regular video clips.
